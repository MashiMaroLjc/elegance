...
class Transfromer:
    ...
    def _up(self, pil_img, up):
        if up > 4:
            raise ValueError("The up params must less than 4")
        up = int(up)
        a, b = pil_img.size
        a = int(a * up)
        b = int(b * up)
        return pil_img.resize((a, b), Image.BICUBIC)

    def get_preprocess(self):
        max_size = self.param.get("maxsize", 1380)
        up = self.param.get("up", 1)
        blur = self.param.get("blur", None)

        def preprocess(pil_img):
            pil_img = max_size_fit(pil_img, int(max_size))
            pil_img = self._up(pil_img, int(up))
            pil_img = down_sample_fit(pil_img, 2)
            if blur :
                r = self.param.get("param", 5)
                pil_img = blur_image(pil_img, str(blur), int(r))
            return pil_img
